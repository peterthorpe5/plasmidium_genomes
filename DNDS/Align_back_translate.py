...
dest_dir = os.path.join(script_dir, 'back_translated')
try:
...
for filename in os.listdir(".") :
    if filename.endswith("refine.fasta"):
        name = filename[:]
        print(filename)
        ...
        nuc_fasta_file = "/storage/home/users/pjt6/plas/Pete/gff/allnt.fasta"
        name_out = "./back_translated/%s.DNA_backtranslated.fasta" % (name[:-5])
...
print("%d failed files!!!!!!!" % (fail_count))
